...
def proper_round(num, dec=1):
    ...
    if num[-1] >= '5':
        return float(num[:-2 - (not dec)] + str(int(num[-2
                     - (not dec)]) + 1))
    return float(num[:-1])
...
class GetStats:
    ...
    def getRam(self):
        ...
        availableRam = psutil.virtual_memory().available * 100 \
            / psutil.virtual_memory().total
        return [{'usedRam': usedRam,
    ...
    def getTemps(self):
        return ('Temperature sensors not supported on current host'
                 if psutil.sensors_temperatures()
                == {} else psutil.sensors_temperatures())
